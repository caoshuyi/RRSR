...
parser = argparse.ArgumentParser(description="PyTorch SR Eval")
parser.add_argument("--cuda", action="store_true", help="use cuda?")
parser.add_argument("--model", default="checkpoint/model_epoch_50.pth",type=str, help="model path")
parser.add_argument("--dataset", default="data/Set5", type=str, help="dataset name, Default: Set5")
parser.add_argument("--inputimg", default="data/Set5/butterfly_GT.bmp", type=str, help="input name")
...
for scale in scales:
    ...
    for image_name in image_list:
        if str(scale) in image_name:
            ...
            img = Image.open(opt.inputimg).convert('YCbCr')
            y, cb, cr = img.split()

            out_img_y = HR.data[0].numpy()
            out_img_y *= 255.0
